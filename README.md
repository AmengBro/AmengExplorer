...
- **双面板模式**：并排浏览两个目录，支持同步导航；左右面板可独立切换列表/网格视图
...
- **批量操作**：支持多选（Ctrl+Click）、全选（`Ctrl+A`），批量执行复制/移动/删除
- **多选视觉反馈**：选中文件高亮显示，Ctrl+Click 可逐步添加或取消选择

### 内置回收站
- **程序内置**：虚拟根下 `root\root\Trash`（root）/ `root\home\<其他用户>\Trash` 作为自定义回收站（当前用户来自 `root/etc/users.toml`），不依赖系统回收站
- **智能删除**：Delete 键在普通目录中移动文件到回收站，在回收站内执行永久删除
- **恢复支持**：右键回收站中的文件可选择"恢复"到原位置
- **冲突处理**：同名文件自动添加后缀避免覆盖
...
### 辅助功能
- **文件夹大小计算**：后台 Worker 计算文件夹大小，支持暂停/取消/恢复；完成或取消后可手动清除
- **任务进度面板**：浮窗展示所有进行中/已完成任务，支持批量清除已完成与已取消任务，单个任务也可独立移除
- **快速预览**：悬停或选中文件时显示属性信息
- **命令面板**：`Ctrl+P` 快速执行命令
- **设置面板**：左下角设置按钮，提供通用、搜索、外观、关于四个标签页
- **深色主题**：默认深色界面
...
├── config/
│   ├── icons.json            # 图标映射配置
│   ├── settings.json         # 应用设置（主题、视图等）
│   └── launchpad-history.json # 启动台历史记录
├── root/
│   └── etc/
│       └── fstab             # 盘符挂载配置
├── src/
...
│   │   └── style.css         # 全局样式
│   ├── js/
│   │   ├── app.js            # 主应用逻辑（UI、设置面板、回收站）
│   │   ├── virtual-fs.js     # 虚拟文件系统封装（含回收站路径）
│   │   ├── amsys-client.js   # amsys.exe 管道通信客户端
│   │   ├── icons.js          # 图标渲染引擎
│   │   ├── user-config.js    # 用户路径解析
│   │   └── search-index-worker.js # 文件索引 Worker
│   ├── index.html            # 主页面
